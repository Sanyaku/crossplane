...
func getResourceChildrenRefs(r *resource.Resource, getConnectionSecrets bool) []v1.ObjectReference {
	...
	// We can determine if it's a modern XR by checking for the presence of the crossplane field
	out := &reference.Claim{}
	if err := fieldpath.Pave(obj.Object).GetValueInto("spec.crossplane", out); err == nil {
		xr.Schema = composite.SchemaModern
	}
	xrRefs := xr.GetResourceReferences()

	if xr.Schema == composite.SchemaModern {
